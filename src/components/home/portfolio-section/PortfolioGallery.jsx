      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-10">
        {projects.map((project, index) => (
          <div key={index} className="relative group  overflow-hidden shadow-lg">
            ...
            <div className="absolute inset-0 p-5 m-5 flex flex-col justify-end opacity-0 group-hover:opacity-70 bg-white transition-all duration-500 backdrop-blur-md translate-y-8 group-hover:translate-y-0">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="bg-gray-100 border border-gray-300 text-sm text-gray-700 px-2 py-1 rounded"
                  >
                    {tag}
                  </span>
                ))}
